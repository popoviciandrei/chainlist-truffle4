...
contract('ChainList', function(accounts) {
  ...
  it('should be intialized with empty values', function() {
    return ChainList.deployed()
      ...
      .then(function(data) {
        assert.equal(data.toNumber(), 0, 'no of articles must be 0');
  ...
  it('should let us sell a first article', function() {
    return ChainList.deployed()
      ...
      .then(function(receipt) {
        assert.equal(receipt.logs.length, 1, 'one event should have benn triggered');
      ...
      .then(function(data) {
        assert.equal(data, 1, 'number of article should be 1');
        return chainListInstance.getArticlesForSale();
      })
      .then(function(data) {
      ...
      .then(function(data) {
        ...
        assert.equal(
          data[4],
          articleDescription1,
          'event article name must be ' + articleDescription1
        );
        assert.equal(
          data[5].toNumber(),
          web3.toWei(articlePrice1, 'ether'),
  ...
  // sell a 2nd article
  it('should let us sell a 2nd article', function() {
    return ChainList.deployed()
      .then(function(instance) {
        chainListInstance = instance;
        return chainListInstance.sellArticle(
      ...
      .then(function(data) {
        assert.equal(data, 2, 'number of article should be 2');
        return chainListInstance.getArticlesForSale();
      })
      .then(function(data) {
      ...
      .then(function(data) {
        assert.equal(data[0].toNumber(), 2, 'id must be 2');
        assert.equal(data[1], seller, 'event seller must be ' + seller);
        assert.equal(data[2], 0x0, 'buyer must be empty');
        assert.equal(data[3], articleName2, 'event article name must be ' + articleName1);
        assert.equal(
          data[4],
          articleDescription2,
          'event article name must be ' + articleDescription2
        );
        assert.equal(
          data[5].toNumber(),
          web3.toWei(articlePrice2, 'ether'),
    ...
    return ChainList.deployed().then(function(instance) {
      ...
      return chainListInstance
        ...
        .then(function(receipt) {
          ...
          // record balance of buyer & seller after the purchase
          sellerBalanceAfterBuy = web3.fromWei(web3.eth.getBalance(seller), 'ether').toNumber();
          buyerBalanceAfterBuy = web3.fromWei(web3.eth.getBalance(buyer), 'ether').toNumber();

          // check the effect of buy on balance of buyer & seller, accounting for gas
          assert(
          ...
          assert(
            ...
            `buyer should have lost ${articlePrice1} ETH`
          );
          return chainListInstance.getArticlesForSale();
        })
        .then(function(data) {
