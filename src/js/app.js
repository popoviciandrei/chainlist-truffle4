App = {
  ...
  initWeb3: function() {
    if (typeof web3 !== 'undefined') {
      // reuse the provider of the web3 injected by Metamask
  ...
  initContract: function() {
    $.getJSON('ChainList.json', function(chainListArtifact) {
      // get the contract artifact and use it to instantiate the contract abstraction
    ...
    App.contracts.ChainList.deployed().then(function(instance) {
      return instance
        ...
        .then(function(result) {
          // this methods not needed as it is used in the "listenToEvents"
          //App.reloadArticles();
        })
        .catch(function(err) {
    ...
    App.contracts.ChainList.deployed().then(function(instance) {
        ...
        if (!err) {
          $('#events').append(
            `<li class="list-group-item">
                ${event.args._name} is now for sale
            </li>`
          );
        } else {
          console.error(err);
      ...
      instance.LogBuyArticle({}, {}).watch(function(err, event) {
        if (!err) {
          $('#events').append(
            `<li class="list-group-item">
                ${event.args._buyer} bought ${event.args._name} 
            </li>`
          );
        } else {
          console.error(err);
